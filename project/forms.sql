...
(
    ...
    PRIMARY KEY (id),
    FOREIGN KEY (creator) REFERENCES pi_users (id)
);

CREATE TABLE pi_question_type
(
    id   INT(10) UNSIGNED AUTO_INCREMENT NOT NULL,
...
(
    ...
    settings       VARCHAR(32)                     NULL DEFAULT NULL,

    title          VARCHAR(64)                     NOT NULL,
    comment        VARCHAR(256)                         DEFAULT NULL,

    answers        VARCHAR(1024)                        DEFAULT NULL,
    comment_answer VARCHAR(256)                         DEFAULT NULL,

    PRIMARY KEY (id),
    FOREIGN KEY (form) REFERENCES pi_forms (id),
    FOREIGN KEY (type) REFERENCES pi_question_type (id)
);

CREATE TABLE pi_solutions
(
    id           INT(10) UNSIGNED AUTO_INCREMENT NOT NULL,
    form         INT(10) UNSIGNED                NOT NULL,
    ...
    validate_key VARCHAR(256) DEFAULT NULL,

    PRIMARY KEY (id),
    FOREIGN KEY (form) REFERENCES pi_forms (id),
    FOREIGN KEY (user) REFERENCES pi_users (id)
);
...
(
    ...
    answers        VARCHAR(1024) DEFAULT NULL,
    comment_answer VARCHAR(1024) DEFAULT NULL,

    PRIMARY KEY (id),
    FOREIGN KEY (question) REFERENCES pi_questions (id)
);


INSERT into pi_question_type (type)
VALUES ('single'),
       ('multi'),
       ('text'),
       ('numeric');
...
delimiter $$
CREATE PROCEDURE DeleteForm(form_ int)
BEGIN
    IF NOT EXISTS(SELECT * FROM pi_questions WHERE pi_questions.form = form_) THEN
DELETE FROM pi_forms WHERE pi_forms.id = form_;
END IF;
END $$
delimiter ;